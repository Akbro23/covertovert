...
class MyCovertChannel(CovertChannelBase):
    ...
    def send(self, log_file_name, packets_per_bit, bit_one_threshold):
        '''
        Encodes and transmits a message as TCP packets using the FIN flag.

        Parameters
        ----------
        log_file_name : str
            The name of the log file where the original message will be written. 
            The message ends with dot ``.`` indicating its end.

        packets_per_bit : int
            The number of packets used to encode each bit of the original message. 
            This defines how many bits are accumulated before they are compared and 
            decoded into a bit (``0`` or ``1``).

        bit_one_threshold : int
            The threshold value used to determine whether a set of accumulated bits 
            represents a ``0`` or ``1``.
        ...
        -------
        None
            This function does not return any value. Instead, it transmits the encoded message as TCP packets
            and does not stop until the message has been completely sent.
        ...
        assert 0 < bit_one_threshold < 2 ** packets_per_bit, 'Threshold must satisfy the constraint 0 < `bit_one_threshold` < 2 ^ `packets_per_bit`'
    ...
    def receive(self, log_file_name, packets_per_bit, bit_one_threshold):
        '''
        Decodes a message transmitted through TCP packets using the FIN flag and logs the message to a file.

        Parameters
        ----------
        log_file_name : str
            The name of the log file where the final decoded message will be written. 
            The message is logged once a dot ``.`` is received in the message.
        packets_per_bit : int
            The number of packets used to encode each bit of the original message. 
            This defines how many bits are accumulated before they are compared and 
            decoded into a bit (``0`` or ``1``).
        ...
        -------
        None
            This function does not return any value. Instead, it logs the decoded 
            message to a file and stops when a dot character is encountered.

            
        This function uses the following variables to decode the transmitted message:
         - ``bits``: Accumulates FIN bits (from TCP packets) to reconstruct the original message's bits.
         - ``byte``: Accumulates 8 bits of the original message to form a byte (character).
